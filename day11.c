...
char output[105][105];// becouse i thinks its difiicult to change seats on the run

int occupied=0;// if >4 no seat.
...
int main()
{
    ...
    //output=input;// why doesn't C support this.
    
    while(changed)// while process is running
    {
        changed=0;
        for(i=0;i<100;i++)
        {
            for(j=0;j<105;j++)//y
            {
                ...
                {
                    changed=1;// if once changed it's not stable yet.
                }
